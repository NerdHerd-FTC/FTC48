...
@Autonomous(name = "Back Board Right Side V2")
public class _2023112202_Back_Board_Right_Side_Parking_V2 extends LinearOpMode {
    ...
    double speed = 0.2;
    double reverseSpeedForBraking = 0.075;
    int brakingLength = 1250;

    double targetDistanceApproachSpeed = 0.1;

    int distance = 24;

    double distanceToTargetForDecceleration = 0.75;

    double percentDistanceBeforeDecceleration = 0;
    ...
    public void runOpMode() throws InterruptedException {
        ...
        if (opModeIsActive()) {
            ...
            update();
            telemetry.addData("Measured Distance at End: ", (calculateOdometryInches(rightOdometryPod.getCurrentPosition())+calculateOdometryInches(leftOdometryPod.getCurrentPosition()))/2);
            telemetry.update();
            sleep(5000);
        }
    }
    ...
    public double convertInchesToTicks(double inches) {
        double inchesConvertedToTicks = (inches/5.9368686994184620294901188105598) * 2000;
        return inchesConvertedToTicks;
    ...
    public void moveForward(double speed, double inches) {
        ...
        int targetTicks = (int) Math.round(convertInchesToTicks(inches));
        ...
        while (true) {
            ...
            int odometryReadingAverage = Math.round((rightOdometryPodTicks + leftOdometryPodTicks)/2);

            telemetry.addData("Odometry Reading Average: ", "%d ticks", odometryReadingAverage);
            ...
            if (odometryReadingAverage > (distanceToTargetForDecceleration*targetTicks) && odometryReadingAverage < targetTicks) {
                frontRightMotor.setPower(targetDistanceApproachSpeed);
                frontLeftMotor.setPower(targetDistanceApproachSpeed);
                backRightMotor.setPower(targetDistanceApproachSpeed);
                backLeftMotor.setPower(targetDistanceApproachSpeed);

            } else if (odometryReadingAverage >= targetTicks) {
                frontRightMotor.setPower(0);
                frontLeftMotor.setPower(0);
                backRightMotor.setPower(0);
                backLeftMotor.setPower(0);
